...
#            self.chain if not self.chains_to_keep else "( %s or %s )"%(self.chain,"c. "+(" or c. ".join(self.chains_to_keep))) ))
    ...
    """def mapping(self):
        ...
        str_sequence = self.str_sequence  # "".join([seq1(x.name) for x in self.residues])
        (astr, aseq), _ = nw(str_sequence, self.sequence)
        mismatch = any([a != b and a not in [".", "-"] and b not in [".", "-"] for (a, b) in zip(astr, aseq)])
        blaseq = aseq
        blastr = astr
        if mismatch:
            mmwindow = Tk.Toplevel()
            e = Tk.Text(mmwindow, width=80)
            e.insert(Tk.END, "There is a mismatch in the sequence for {} ({}).\n
            Aligned sequences:\n
            From fasta: \n{}\n
             From .pdb: \n{}
            ".format(self.seqName, self.objId, aseq, astr))
            e.config(state='disabled')
            e.pack(side=Tk.TOP)
            def _mm_quit(*args):
                aseq = blaseq
            ...
            #tkMessageBox.showinfo("FYI", "There is a mismatch in the sequence for {} ({}).\n
            #Aligned sequences:\n
            #From fasta: {}\n
            # From .pdb: {}
            #".format(self.seqName, self.objId, aseq, astr))

        self.sequence_residues = [False for i in self.sequence if
                                  i not in ".qwertyuiopasdfghjklzxcvbnm"]  # point to self.residue[x] corresponding to xth index in DI scores
        str_pointer = -1
        dca_pointer = -1
        for i, (fres, sres) in enumerate(zip(aseq, astr)):
            if sres != "-":
                str_pointer += 1
            if fres not in ".qwertyuiopasdfghjklzxcvbnm-":  # not insert
                dca_pointer += 1
                if sres != "-":
                    self.sequence_residues[dca_pointer] = str_pointer
                    self.residues[str_pointer].seqid = dca_pointer
            else:
                if sres != "-":
                    self.residues[str_pointer].insert = True
                    self.residues[str_pointer].seqid = False
            if sres != "-" or fres not in ".-":
                self.residues[str_pointer].SSid = len(self.residues)
                tmp = []
                if fres not in ".qwertyuiopasdfghjklzxcvbnm-":
                    tmp.append([dca_pointer])
                else:
                    tmp.append(False)
                if sres != "-":
                    tmp.append([self.residues[str_pointer].pdbid])
                    tmp.append(self.residues[str_pointer].ss if self.residues[str_pointer].ss else "-")
                else:
                    tmp.append(False)
                    tmp.append(".")"""
